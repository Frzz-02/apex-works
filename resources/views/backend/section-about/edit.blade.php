...
<div class="p-6">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-slate-800">Edit About Section</h1>
    </div>

    <form action="{{ route('backend.section-about.update', $sectionAbout->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $sectionAbout->title) }}" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Subtitle</label>
                            <input type="text" name="subtitle" value="{{ old('subtitle', $sectionAbout->subtitle) }}" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Blockquote</label>
                            <input type="text" name="blockquote" value="{{ old('blockquote', $sectionAbout->blockquote) }}" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Description</label>
                            <textarea name="description" rows="4" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $sectionAbout->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="space-y-4">
                        @foreach(range(1, 3) as $i)
                            @php $field = 'checklist_' . $i; @endphp
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Checklist {{ $i }}</label>
                                <input type="text" name="{{ $field }}" value="{{ old($field, $sectionAbout->$field) }}" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <input type="text" name="video_url" value="{{ old('video_url', $sectionAbout->video_url) }}" placeholder="https://youtube.com/..." class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="space-y-3">
                        @foreach(range(1, 18) as $i)
                            @php $field = $i === 1 ? 'image' : 'image_' . $i; @endphp
                            <div>
                                <label class="block text-sm font-medium text-slate-600 mb-1">Image {{ $i }}</label>
                                @if($sectionAbout->$field)
                                    <img src="{{ asset('storage/' . $sectionAbout->$field) }}" alt="Image {{ $i }}" class="w-full h-24 object-cover rounded-lg mb-1">
                                @endif
