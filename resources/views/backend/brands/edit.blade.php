            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    @error('logo')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Upload new logo to replace existing (JPG, PNG, GIF - Max: 2MB)</p>
                    @if($brand->logo)
                        <div class="mt-3">
                            <p class="text-xs text-gray-600 mb-2">Current Logo:</p>
                            <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="h-16 w-auto object-contain border border-gray-200 rounded p-2">
                        </div>
                    @endif
