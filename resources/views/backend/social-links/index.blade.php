...
<div class="p-8">
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('backend.social-links.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold rounded-lg shadow-md transition-all duration-300">
                    ...
                    @forelse($socialLinks as $link)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                @if($link->icon)
                                    <i class="{{ $link->icon }} text-xl text-indigo-600"></i>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    <a href="{{ route('backend.social-links.edit', $link) }}" class="text-amber-600 hover:text-amber-900 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('backend.social-links.destroy', $link) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this social link?');">
                                        @csrf
                                        @method('DELETE')
